...
def create_label_noise(train_image_list, noise_type, noise_rate, nb_classes, random_state=0):
    train_labels = []
    for e in train_image_list:
        train_labels.append(e[0])

    train_labels=np.asarray([[train_labels[i]] for i in range(len(train_labels))])

    train_noisy_labels, actual_noise_rate = noisify(dataset='cifar10', train_labels=train_labels, noise_type=noise_type, noise_rate=noise_rate, random_state=random_state, nb_classes=nb_classes)
    ...
    train_noisy_image_list = []
    for i in range(0, len(train_image_list)):
        k,e = train_image_list[i]
        train_noisy_image_list.append((train_noisy_labels[i], e, noise_or_not[i]))

    return train_noisy_image_list
...
class ImageFolder2(data.Dataset):
    def __init__(self, transform=None, image_pathes=None, loader=pil_loader):
        self.transform = transform
        self.image_pathes = image_pathes
        self.loader = loader
        self.imgs = []
        self.labels = []
        for e in image_pathes:
            self.imgs.append(e[1])
            self.labels.append(e[0])

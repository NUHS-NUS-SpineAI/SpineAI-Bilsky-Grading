...
train_dir = '/hdd8/zhulei/spine-mets/Jul012021_UpdatedTrainTestSplitandLabels/PreprocessedVersion_MedoidPoint20_Jul062021_combine_mild_normal/train'
val_dir = '/hdd8/zhulei/spine-mets/Jul012021_UpdatedTrainTestSplitandLabels/PreprocessedVersion_MedoidPoint20_Jul062021_combine_mild_normal/val'

folders = ['normal', 'b1a', 'b1b', 'b1c', 'b2', 'b3']
...
val_loader = torch.utils.data.DataLoader(ImageFolder2(transform_test, image_list_val), batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
...
def train(epoch, lr_scheduler=None):
    ...
    for batch_idx in range(0, iteration):
        ...
        # count_pix_value += np.sum(np.square(inputs.detach().cpu().numpy()-0.23393838), (0,2,3))
        # count_pix_value += np.sum(inputs.detach().cpu().numpy(), (0,2,3))
...
def val(epoch):
    ...
    print('acc,', 100.*(np.trace(confusion_matrix)/np.sum(confusion_matrix)))
    ...
    if store_weights:
        ...
        if not os.path.isdir('/hdd8/zhulei/spine-mets/checkpoint-july-2021'):
            os.mkdir('/hdd8/zhulei/spine-mets/checkpoint-july-2021')

        torch.save(state1, '/hdd8/zhulei/spine-mets/checkpoint-july-2021/data-july-2021-20-with-mild-model-'+str(args.kth)+'-'+str(epoch)+'.t7')
...
for epoch in range(start_epoch, args.n_epoch):
    ...
    with open('./records/data-july-2021-20-with-mild-record-sgd-'+str(args.lr)+'-'+str(args.kth)+'-'+str(args.lamb)+'-'+str(args.temp)+'-'+str(args.n_epoch)+'-'+args.note+'.txt', "a") as myfile:
...
with open('./record-all.txt', 'a') as f:
    f.write('data-july-2021-20-with-mild-'+str(args.kth)+'-sgd-'+str(args.lr)+'-'+args.note+'-'+str(args.lamb)+'-'+str(args.temp)+'-'+str(last_acc)+'-'+str(best_acc)+'-'+str(last_avg_acc)+'-'+str(best_avg_acc)+'\n')


# # code for test
# store_weights = False
# checkpoint = torch.load('/hdd8/zhulei/spine-mets/checkpoint-july-2021/data-july-2021-orig-with-mild-model-103-42.t7')
# model.load_state_dict(checkpoint['net'])

# test_dir = '/hdd8/zhulei/spine-mets/Jul012021_UpdatedTrainTestSplitandLabels/PreprocessedVersion_OriginalCrop_Jul062021_combine_mild_normal/test'
